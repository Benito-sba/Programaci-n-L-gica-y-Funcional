...
#    R: Es una función que se puede usar igual que una variable. Se puede guardar, 
#     enviar como parámetro o regresar desde otra función.
#
# 2. ¿Cuál es la diferencia entre una función de orden superior y un callback?
#    R: La función de orden superior es la que recibe otra función, y 
#       el callback es la función que se manda para que haga una tarea.
#
# 3. ¿Cuándo conviene usar comprensión de listas en lugar de un ciclo for?
#    R: Cuando quieres crear una lista de manera más rápida y corta, siempre que 
#       el proceso no sea muy complicado.
#
# 4. ¿Qué hace map() y cómo se relaciona con lambda?
#    R: sirve para aplicar una operación a todos los elementos de una lista. 
#       lambda ayuda a escribir esa operación de forma rápida y sencilla.
#
# 5. ¿Qué ventaja ofrece pasar una función como argumento a otra función?
#    R: Permite que el código sea más flexible y reutilizable, porque una 
#      función puede hacer diferentes tareas según la función que reciba.
# ─────────────────────────────────────────────────────────────────────────────
...
#
# - preparar_pizza() -> prepara una pizza
# - preparar_agua() -> prepara un agua fresca
# - preparar_tamal() -> prepara un tamal
# - tomar_orden() -> recibe la función del producto y la cantidad
# - calcular_total() -> calcula el precio total del pedido
# - aplicar_promocion() -> agrega “postre gratis” si aplica
# - mostrar_resumen() -> muestra el resumen final del pedido
#
# ¿Cuál de ellas es de orden superior? ¿Por qué?
#
# - tomar_orden() es una función de orden superior
#   porque recibe otra función como parámetro.
#
# ¿Dónde usarás comprensión de listas?
#
# - En tomar_orden(), para crear una lista con todas
#   las porciones solicitadas de un producto.
#
# ¿Dónde usarás lambda + map()?
#
# - En calcular_total(), para calcular el precio
#   de cada producto usando map() y lambda.
#
# - Después se puede usar sum() para obtener
#   el total final.
# ─────────────────────────────────────────────────────────────────────────────
...
def tomar_orden(preparar_alimento, cantidad, precio_unitario):
    # a) Comprensión de listas: genera una lista llamando a la función 'cantidad' veces
    porciones = [preparar_alimento() for _ in range(cantidad)]
    
    # b) map() + lambda para precios: asigna el precio unitario a cada elemento
